...
async def add_new_filter(new_handler):
    ...
    if add_filter(str(new_handler.chat_id), keyword, string, msg_id) is True:
        return await edit_or_reply(new_handler, success.format(keyword, "ditambahkan"))
    remove_filter(str(new_handler.chat_id), keyword)
    if add_filter(str(new_handler.chat_id), keyword, string, msg_id) is True:
        return await edit_or_reply(new_handler, success.format(keyword, "diperbarui"))
    await edit_or_reply(new_handler, f"Error saat setting untuk filter {keyword}")
...
async def on_snip_list(event):
    if event.fwd_from:
        return
    OUT_STR = "Tidak ditemukan Filters di chat ini"
    filters = get_filters(event.chat_id)
    for filt in filters:
        if OUT_STR == "Tidak ditemukan Filters di chat ini":
            OUT_STR = "Active filters di chat ini:\n"
        OUT_STR += "👉 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def remove_a_filter(r_handler):
    ...
    if not remove_filter(r_handler.chat_id, filt):
        ...
        await r_handler.edit("Filter `{} `Berhasil di hapus".format(filt))
...
async def on_all_snip_delete(event):
    ...
    if filters:
        ...
        await edit_or_reply(event, f"filters di chat ini berhasil di hapus")
    else:
        await edit_or_reply(event, f"Tidak ditemukan filters di grup ini")
